...
describe("Vault.liquidateLongPosition", function () {
  ...
  it("liquidate long", async () => {
    ...
    expect(await slpManager.getAumInUsdg(false)).eq("99064997000000000000") // 99.064997
    expect(await slpManager.getAumInUsdg(true)).eq("101418485000000000000") // 101.418485
    ...
    expect(await slpManager.getAumInUsdg(false)).eq("101522097000000000000") // 101.522097
    expect(await slpManager.getAumInUsdg(true)).eq("114113985000000000000") // 114.113985
