...
async function getCoreTestnetValues(signer) {
  const provider = new ethers.providers.JsonRpcProvider(CORE_TESTNET_URL)
  const capKeeperWallet = new ethers.Wallet(CORE_TESTNET_DEPLOY_KEY).connect(provider)
  ...
  const shortsTracker = await contractAt("ShortsTracker", "0x2049E86FA9d076f3c0B70a9991934272af9Fb6c5", signer)
...
async function getValues(signer) {
  ...
  if(network === "core-testnet") {
    return getCoreTestnetValues(signer)
  }
}
