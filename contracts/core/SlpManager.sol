...
contract SlpManager is ReentrancyGuard, Governable, ISlpManager {
    ...
    constructor(address _vault, address _usdg, address _slp, address _shortsTracker, uint256 _cooldownDuration) public {
        gov = msg.sender;
        vault = IVault(_vault);
        usdg = _usdg;
        slp = _slp;
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external override onlyGov {
        require(shortsTrackerAveragePriceWeight <= BASIS_POINTS_DIVISOR, "SlpManager: invalid weight");
    ...
    function setCooldownDuration(uint256 _cooldownDuration) external override onlyGov {
        require(_cooldownDuration <= MAX_COOLDOWN_DURATION, "SlpManager: invalid _cooldownDuration");
        cooldownDuration = _cooldownDuration;
    }
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minSlp) private returns (uint256) {
        ...
        uint256 mintAmount = aumInUsdg == 0 ? usdgAmount : usdgAmount.mul(slpSupply).div(aumInUsdg);
        ...
        emit AddLiquidity(_account, _token, _amount, aumInUsdg, slpSupply, usdgAmount, mintAmount);

        return mintAmount;
    }

    function _removeLiquidity(address _account, address _tokenOut, uint256 _slpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        require(_slpAmount > 0, "SlpManager: invalid _slpAmount");
        require(lastAddedAt[_account].add(cooldownDuration) <= block.timestamp, "SlpManager: cooldown duration not yet passed");
