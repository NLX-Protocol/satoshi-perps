 ...
 * @title StorkOracle
 * @author Venus
 * @notice This oracle fetches prices of assets from the Stork oracle.
 */
contract StorkOracle is AccessControlledV8, OracleInterface {
    ...
    /**
     * @notice Gets the price of a asset from the stork oracle
     * @param asset Address of the asset
     * @return Price in USD from Stork or a manually set price for the asset
     */
    ...
    /**
     * @notice Gets the Stork price for a given asset
     * @param asset address of the asset
    ...
    /**
     * @notice Get the Stork price for an asset, revert if token config doesn't exist
     ...
     * @custom:error Price error is thrown if the Stork price of asset is not greater than zero
    ...
    function _getStorkPrice(
        ...
        // Stork USD-denominated feeds store answers at 8 decimals, mostly
        uint256 _priceDecimals = feed.decimals();

        (, int256 answer, , uint256 updatedAt, ) = feed.latestRoundData();
        if (answer <= 0) revert("stork price must be positive");
        ...
        if (deltaTime > maxStalePeriod) revert("stork price expired");
